...
short red = 255;  //0-255
float rlight= 1;
short green = 255;  //0-255
float glight=0;
short blue = 255;  //0-255
float blight=1;
short blackPoint = 20;  //minimum value for rgb

/*Sun Color*/
color sunColor = #ffccaa;
...
void draw() {
  ...
  fill(255);
  textSize(18);
  text("Points: " + Level.level,10,20);
  if (tempo_feedback == 0) fill(255,0,0);
  else fill(0);
  circle(width/2 - 30, 20, 20);
  if (tempo_feedback == 1) fill(0, 255,0);
  else fill(0);
...
void serialEvent(Serial serial) {
  ...
  if (serialIn != null) {
    try {
      ...
      if (command.equals("Y-")) bpm_multiplier = int(map(param, 0, 255, 8, 1));
      if (command.equals("X-")) starspeed_multiplier = map(param, 0, 255, 0.3, 4);
      if (command.equals("T-")) {
        if (isBpmGood(bpm, param) == true) {
          Level.hit();
          tempo_feedback = 1;
        } else {
          if (param < bpm) tempo_feedback = 0;
          else tempo_feedback = 2;
          if (param == 0) Level.miss(10);
          else Level.miss();
        }
      }
    } 
...
void oscEvent(OscMessage theOscMessage) {
  if (theOscMessage.checkAddrPattern("/tempo")==true) {
    ...
    triborders = int(map(oscReceive(theOscMessage), 0, 1, 255, 0));
